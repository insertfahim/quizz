...
import { ArrowLeft, Edit, Users, Clock, Calendar, BarChart3, CheckCircle, XCircle, TrendingUp } from "lucide-react";
...
export default function QuizDetailsPage() {
  ...
  const averageScore = quiz.submissions.length > 0
    ? quiz.submissions.reduce((sum, s) => sum + s.score, 0) / quiz.submissions.length
    : 0;

  const averageTime = quiz.submissions.length > 0
    ? quiz.submissions
        .filter(s => s.timeSpent)
        .reduce((sum, s) => sum + (s.timeSpent || 0), 0) / quiz.submissions.filter(s => s.timeSpent).length
    : 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button
            ...
            className={quiz.isActive ? "text-red-600 hover:text-red-700" : "text-green-600 hover:text-green-700"}
          >
            {quiz.isActive ? (
              <>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="text-center p-4 bg-green-50 rounded-lg">
              <div className="text-2xl font-bold text-green-600">{quiz._count.submissions}</div>
            ...
            <div className="text-center p-4 bg-orange-50 rounded-lg">
              <div className="text-2xl font-bold text-orange-600">
                {averageTime > 0 ? `${Math.round(averageTime / 60)}m` : "N/A"}
              </div>
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <p className="mt-1 text-gray-600">{quiz.description || "No description provided"}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="mt-1">
                <span className={`inline-flex items-center gap-1 ${quiz.isActive ? 'text-green-600' : 'text-gray-600'}`}>
            ...
            <div>
              ...
              <p className="mt-1 text-gray-600">{new Date(quiz.createdAt).toLocaleDateString()}</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              {quiz.questions.map((question, index) => (
                <div key={question.id} className="border rounded-lg p-4">
                  ...
                  <div className="text-xs text-gray-500">
                    Type: {question.type.replace('_', ' ')} • {question.options.length} options
                  </div>
                </div>
              ))}
        ...
        <Card>
          <CardHeader>
            <CardTitle>Recent Submissions ({quiz.submissions.length})</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {quiz.submissions.slice(0, 10).map((submission) => (
                <div key={submission.id} className="border rounded-lg p-3">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      <h4 className="font-medium text-sm">{submission.user.name}</h4>
                      <p className="text-xs text-gray-600">{submission.user.email}</p>
                    </div>
                    <div className="text-right">
                      <div className="text-sm font-medium">{submission.score.toFixed(1)}%</div>
                      <div className="text-xs text-gray-500">
